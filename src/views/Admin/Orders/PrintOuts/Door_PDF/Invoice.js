  ...
  return [
    ...
    data.part_list.map((part, i) => {
      ...
      part.dimensions.forEach((item, index) => {
        ...
        tableBody.push([
          ...
          { text: `${(data.linePrice[i][index] / parseInt(item.qty)).toFixed(2)}`, style: 'fonts' },
    ...
    },
    {
      columns: [
        ...
        { text: data.misc_items.map(i => { return `$${i.price ? (i.price).toFixed(2) : i.pricePer ? (parseFloat(i.pricePer)).toFixed(2) : ''} \n`; }), style: 'fonts', margin: [0, 0, 0, 0] },
