...
const AppliedProfiles = (props) => {
  ...
  const changeFilterValue = (e) => {
    setFilteredProducts(
      props.applied_profiles.filter((i) =>
        i.NAME.split(' ')
          .join('')
          .toLowerCase()
      ...
      >
        <Card style={{ height: '100%' }} onClick={() => setCard(card)}>
          ...
          <CardBody>
            ...
            <CardTitle>
              <strong>Rail Factor: </strong> {card.RAIL_FACTOR}
            </CardTitle>
          </CardBody>
        </Card>
      </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(AppliedProfiles);
